...
class Env(object):
    ...
    def setReward(self, state, action, done):
        ...
        if self.get_goalbox:
            ...
            if self.num_goal == 1:
                self.goal_x, self.goal_y = 0, -4
            elif self.num_goal == 2:
                self.goal_x, self.goal_y = 0, -3
            elif self.num_goal == 3:
                self.goal_x, self.goal_y = 4,-1.5
                self.reset()
            else:
    ...
    def step(self, action, past_action):
        ...
        state, done = self.getState(data, past_action)  # state 是列表 长度为28
        reward, done ,goal_flag= self.setReward(state, action, done)  # reward=4.02

        return np.asarray(state), reward, done ,goal_flag # 状态、奖励、是否碰撞
    ...
    def reset(self):
        ...
        # 获取到目标点的距离
        self.goal_distance = self.getGoalDistace()
        state, done = self.getState(data, [0] * self.action_dim) # 传入data 和 一个列表推倒式
        #len(state)=28
